...
2. 在生成对话时，参照用户的单词本信息优先融入掌握比例较低的短语对应的含义（例如熟练度< ${masteryLevel}），同时融入单词本中不存在的生词，生词需要满足当前用户的词汇水平，生词的比例不超过对话总词汇的${newWordsPercent}%，确保对话自然。每个对话回合应尽可能包含至少一个生词或短语，但避免强行插入，保持对话流畅。
