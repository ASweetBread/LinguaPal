  ...
  return (
    <>
          ...
          <CardContent className="space-y-6">
            {loading ? (
              ...
              <>
                ...
                {/* 提交按钮 */}
                <Button onClick={handleSubmitNormal} disabled={loading || !userInput.trim()} className="w-full">
                  提交测试结果
                </Button>
              </>
            )}
      ...
      {mode === LEARNING_MODES.PROMPT && (
        <PromptDisplay prompt={prompt} onSubmit={handleAnalysisResultSubmit} onClose={() => onOpenChange(false)} />
      )}
    </>
  );
};
