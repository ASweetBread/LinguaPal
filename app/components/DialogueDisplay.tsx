...
export default function DialogueDisplay() {
  const { dialogue, vocabulary, currentSentenceIndex, setCurrentSentenceIndex, setShowPractice } = useDialogueStore();
  ...
  const [localPracticeStates, setLocalPracticeStates] = useState<
    Record<number, { passed: boolean | null; recognizedText: string }>
  >({});
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div className="space-y-6">
        {dialogue.map((item, index) => {
          ...
          return (
            <div key={index} className={`rounded-lg shadow-sm overflow-hidden ${bgColorClass} transition-colors`}>
              <div className={`px-4 py-2 ${role === "A" ? "bg-blue-100" : "bg-green-100"}`}>
                <span className={`font-medium ${role === "A" ? "text-blue-700" : "text-green-700"}`}>角色 {role}</span>
              ...
              <div className="p-4">
                ...
                {sentenceState.passed !== null && (
                  <div
                    className={`mb-3 p-2 rounded-md text-sm font-medium ${sentenceState.passed ? "bg-green-100 text-green-800 dark:text-black" : "bg-red-100 text-red-800 dark:text-black"}`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {vocabulary.map((item, index) => (
                <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                  ...
                  {item.partOfSpeech && <div className="text-sm text-blue-600 mb-2">{item.partOfSpeech}</div>}
                  ...
                  {item.phrase && (
                    <div className="mt-2 p-2 bg-gray-50 rounded">
                      ...
                      {item.phraseMeaning && <div className="text-sm text-gray-600">{item.phraseMeaning}</div>}
                    </div>
                  )}
                </div>
              ))}
