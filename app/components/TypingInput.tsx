...
export default function TypingInput({
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, wordIndex: number) => {
    ...
    if (e.key === " ") {
      ...
      if (wordIndex < totalWords - 1) {
        const nextInput = findInput(wordIndex, "next");
        if (nextInput) {
          nextInput.focus();
        }
      }
    ...
    if (e.key === "Backspace" && !e.currentTarget.value && wordIndex > 0) {
      e.preventDefault();
      const prevInput = findInput(wordIndex, "prev");
      if (prevInput) {
        prevInput.focus();
    ...
    // 处理右箭头键
    if (e.key === "ArrowRight") {
      const totalWords = inputRefs.current.length;
  ...
  const findInput = (currentIndex: number, type: "next" | "prev"): HTMLInputElement | null => {
    const step = type === "next" ? 1 : -1;
    for (let i = currentIndex + step; i >= 0 && i < inputRefs.current.length; i += step) {
    ...
    <div className={`flex flex-wrap items-center ${className}`}>
      {segments.map((segment, idx) => {
        ...
        if (segment.type === "punctuation") {
          return (
            <span key={`punct-${idx}`} className="text-lg text-gray-800 dark:text-gray-200">
              {segment.content}
            </span>
          );
        }
        ...
        if (segment.type === "word") {
          const isPromptWord = promptWord.includes(segment.content);
          let userWord = userWords[segment.index] || "";
          const isFocused = focusedIndex === segment.index;

          if (isPromptWord) {
            userWords[segment.index] = segment.content;
            return (
              <span
                key={`word-${segment.index}`}
                className="text-lg text-gray-800 dark:text-gray-200 border-b-2 border-gray-400 dark:border-gray-600"
              >
                {segment.content}
              </span>
            );
            ...
            <div key={`word-${segment.index}`} className="relative inline-flex flex-col items-center">
              <input
                ...
                className={`
                  ...
                  ${isFocused ? "border-blue-500" : "border-gray-400 dark:border-gray-600"}
                `}
                autoComplete="off"
                autoCapitalize="off"
                spellCheck={false}
